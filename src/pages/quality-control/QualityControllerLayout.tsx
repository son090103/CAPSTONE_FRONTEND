...
export default function QualityControllerLayout() {
  ...
  const avatarUrl = user?.avatar?.trim() || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=256&auto=format&fit=crop';
  const displayName = user?.fullName || 'Quản lý chất lượng';
  const displayRole = 'KCS / QC Staff';

  // Menu items for the sidebar
  const menuItems = [
    { name: 'Kiểm tra chất lượng', icon: ClipboardList, path: '/quality-control/service-orders' },
  ];

  // Dynamic active menu item based on current URL path
  const activeMenu = useMemo(() => {
    const path = location.pathname;
    if (path.includes('/service-orders')) return 'Kiểm tra chất lượng';
    ...
    <>
      ...
      <div className="p-6 border-b border-[#C4E8E0] flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#0E4D40] flex items-center justify-center shadow-lg shadow-[#0E4D40]/10">
            <ShieldCheck size={20} className="text-white" />
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-[#0E4D40] uppercase tracking-wider text-base">AGM Intelligent</span>
            <span className="text-[10px] text-slate-500 font-bold tracking-widest uppercase">Quản lý chất lượng</span>
          </div>
        </div>
        <button
      ...
      <main className="flex-1 flex flex-col min-w-0 pb-16">
        ...
        <header className="hidden md:flex bg-white h-20 px-8 items-center justify-between border-b border-slate-100 shadow-xs sticky top-0 z-25">
          ...
          <div className="relative w-80">
            ...
            <input
              type="text"
              placeholder="Tìm biển số xe, kỹ thuật viên..."
              value={searchQuery}
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col text-right">
                ...
                <span className="text-[11px] text-slate-400 font-bold tracking-wide uppercase">{displayRole}</span>
              </div>
              <div className="relative">
                <img
                  src={avatarUrl}
                ...
                />
                <span className="absolute bottom-0 right-0 w-3 h-3 bg-[#0E4D40] border-2 border-white rounded-full"></span>
              </div>
            </div>
          </div>
        </header>
        ...
        <footer className="mt-auto px-8 py-6 border-t border-slate-200/50 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-semibold text-slate-400">
          <div>
            © 2026 <span className="text-slate-500 font-bold">AGM Intelligent</span> - Hệ thống quản lý chất lượng chuyên nghiệp
          </div>
          <div className="flex items-center gap-6">
